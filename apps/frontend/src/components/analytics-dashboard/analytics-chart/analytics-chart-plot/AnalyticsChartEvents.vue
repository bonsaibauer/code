...
import { ExternalIcon, InfoIcon, TagCategoryFlagIcon } from '@shroudedit/assets'
import { IntlFormatted, useScrollIndicator, useVIntl } from '@shroudedit/ui'
...
const eventGroups = computed<EventGroup[]>(() => {
	const groups = mergeProjectGroupsForShroudEditEventDistance(
...
function mergeProjectGroupsForShroudEditEventDistance(groups: EventGroup[]): EventGroup[] {
	let nextGroups = groups

	while (true) {
		const result = mergeProjectGroupsForShroudEditEventDistanceOnce(nextGroups)
...
function mergeProjectGroupsForShroudEditEventDistanceOnce(groups: EventGroup[]) {
	const clusterLayouts = getCollisionClusterLayouts(groups)

	for (const layout of clusterLayouts) {
		const projectGroupsToMerge = getProjectGroupsToMergeForShroudEditEventDistance(layout)
...
function getProjectGroupsToMergeForShroudEditEventDistance(layout: CollisionClusterLayout) {
	...
	const overLimitPlacement = getMostDisplacedShroudEditPlacement(placements, distanceLimit)
	if (!overLimitPlacement) return []

	const displacedShroudEditPlacement = overLimitPlacement
	const offset = displacedShroudEditPlacement.markerX - displacedShroudEditPlacement.group.x
	const projectPlacementsOnOffsetSide = projectPlacements.filter((placement) =>
		offset > 0
			? placement.index < displacedShroudEditPlacement.index
			: placement.index > displacedShroudEditPlacement.index,
	)
...
function getMostDisplacedShroudEditPlacement(
	...
	for (const placement of placements) {
		if (!isShroudEditEventGroup(placement.group)) continue
...
function isShroudEditEventGroup(group: EventGroup) {
	return group.groupKey === 'shroudedit'
}
