...
import { BookTextIcon, XIcon } from '@shroudedit/assets'
import { ButtonLink, commonMessages, IconButton, PagewideBanner, useVIntl } from '@shroudedit/ui'
	...
	<PagewideBanner v-if="!flags.hideRussiaCensorshipBanner" variant="error">
		<template #title>
			<div class="flex flex-col gap-1 text-contrast">
				<span lang="ru">К сожалению, ShroudEdit скоро станет недоступен в России</span>
				<span class="text-sm font-medium opacity-50" lang="en">
					ShroudEdit will soon be unavailable in Russia
				</span>
			</div>
		</template>
		<template #description>
			<p class="m-0" lang="ru">
				Российское правительство потребовало от нас заблокировать некоторые проекты на ShroudEdit, но
				мы решили отказать им в цензуре.
			</p>
			<p class="-mt-2 mb-0 text-sm opacity-50" lang="en">
				The Russian government has asked us to censor certain topics on ShroudEdit and we have decided
			...
			<p class="-mt-2 mb-0 text-sm opacity-50" lang="en">
				Please seek a reputable VPN or proxy of some kind to continue to access ShroudEdit in Russia.
			</p>
		</template>
		<template #actions>
