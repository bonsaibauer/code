            ...
            async {
                Ok(req.into_response(
                    HttpResponse::Gone()
                        ...
                        .body(r#"{"error":"api_deprecated","description":"You are using an application that uses an outdated version of ShroudEdit's API. Please either update it or switch to another application. For developers: https://docs.shroudedit.com/api/#versioning"}"#)
                ))
            }.boxed_local()
        })
    );
