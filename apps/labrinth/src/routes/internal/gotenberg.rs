...
use crate::util::gotenberg::{
    GeneratedPdfType, SHROUDEDIT_GENERATED_PDF_TYPE, SHROUDEDIT_PAYMENT_ID,
...
pub async fn success_callback(
    ...
    web::Header(ShroudEditGeneratedPdfType(r#type)): web::Header<
        ShroudEditGeneratedPdfType,
    >,
    maybe_payment_id: Option<web::Header<ShroudEditPaymentId>>,
    body: web::Bytes,
...
#[derive(Debug)]
struct ShroudEditGeneratedPdfType(GeneratedPdfType);

impl header::TryIntoHeaderValue for ShroudEditGeneratedPdfType {
    type Error = header::InvalidHeaderValue;
...
impl header::Header for ShroudEditGeneratedPdfType {
    fn name() -> header::HeaderName {
        SHROUDEDIT_GENERATED_PDF_TYPE
    }
    ...
    fn parse<M: HttpMessage>(m: &M) -> Result<Self, ParseError> {
        m.headers()
            ...
            .parse()
            .map_err(|_| ParseError::Header)
            .map(ShroudEditGeneratedPdfType)
    }
}

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct ShroudEditPaymentId(pub PayoutId);

impl header::TryIntoHeaderValue for ShroudEditPaymentId {
    type Error = header::InvalidHeaderValue;
...
impl header::Header for ShroudEditPaymentId {
    fn name() -> header::HeaderName {
        SHROUDEDIT_PAYMENT_ID
    }
