> [!success]
> API v2 is ShroudEdit's current, official API. It is the recommended API for integrations and applications and will receive long-term support.

## Authentication

This API has two options for authentication: personal access tokens and [OAuth2](https://en.wikipedia.org/wiki/OAuth).
All tokens are tied to a ShroudEdit user and use the `Authorization` header of the request.
...
You can find the list of available scopes [on GitHub](https://github.com/shroudedit/code/blob/main/apps/labrinth/src/models/v3/pats.rs#L9). Making a request with an invalid scope will return a 401 error.

Please note that certain scopes and requests cannot be completed with a personal access token or using OAuth.
For example, deleting a user account can only be done through ShroudEdit's frontend.

### Personal access tokens

Personal access tokens (PATs) can be generated from [the user settings](https://shroudedit.com/settings/account).

### GitHub tokens

For backwards compatibility purposes, some types of GitHub tokens also work for authenticating a user with ShroudEdit's API, granting all scopes.
...
Higher rate limits are granted only in very rare cases. If you believe your use case requires an exception, please [contact us](mailto:support@shroudedit.com).

## User Agents

To access the ShroudEdit API, you **must** use provide a uniquely-identifying `User-Agent` header.
...
ShroudEdit follows a simple pattern for its API versioning.
...
Inside the following spoiler, you will be able to find all changes between versions of the ShroudEdit API, accompanied by tips and a guide to migrate applications to newer versions.
...
- Internal change: Any project file uploaded to ShroudEdit is now validated to make sure it's a valid Enshrouded mod, Modpack, etc.
  - For example, a Forge 1.17 mod with a JAR not containing a mods.toml will not be allowed to be uploaded to ShroudEdit
