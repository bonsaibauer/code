...
ShroudEdit uses a small dependency-injection (DI) layer that uses Vue `provide` and `inject`.
...
import { ref } from 'vue'
import { provideMyFeature } from '@shroudedit/ui'
...
import { injectMyFeature } from '@shroudedit/ui'
...
| ---------------------------- | -------------------------------- | ----------------------------- |
| `provideShroudEditClient`      | `providers/api-client.ts`        | Supplies the API client.      |
