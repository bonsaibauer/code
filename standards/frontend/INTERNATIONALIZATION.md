...
Use the `@shroudedit/ui` localization system for all user-visible strings in Vue single-file components (SFCs).
...
Use `defineMessage` or `defineMessages` from `@shroudedit/ui` in `<script setup>`.
...
Use `useVIntl()` from `@shroudedit/ui` to format simple strings:
...
```text
'Welcome to <strong>ShroudEdit</strong>!'
```
...
Use `normalizeChildren` from `@shroudedit/ui` for complex child content:
...
Get all internationalization utilities from `@shroudedit/ui`:
