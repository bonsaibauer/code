...
import { GenericShroudEditClient } from '@shroudedit/api-client'
...
const StorybookProvider = defineComponent({
	setup(_, { slots }) {
		...
		const shroudeditClient = new GenericShroudEditClient({
			userAgent: 'shroudedit-storybook/1.0.0',
		})
		provideShroudEditClient(shroudeditClient)

		const gameVersions = ref<Labrinth.Tags.v2.GameVersion[]>([])
		const loaders = ref<Labrinth.Tags.v2.Loader[]>([])
		shroudeditClient.labrinth.state.build().then((state) => {
