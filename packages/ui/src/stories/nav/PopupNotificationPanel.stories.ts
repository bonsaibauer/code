		...
		setup() {
			...
			const showWarning = () => {
				popupManager.addPopupNotification({
					...
					text: "ShroudEdit App v2.1.0 is available now! Since you're on a metered network, we didn't automatically download it.",
					type: 'warning',
					autoCloseMs: null,
					buttons: [
						{
			...
			const showInfo = () => {
				popupManager.addPopupNotification({
					...
					text: 'ShroudEdit App v2.1.0 has finished downloading. Reload to update now.',
					type: 'info',
					buttons: [
						{
							label: 'Reload',
			...
			const showBlocking = () => {
				popupManager.addPopupNotification({
					contentType: 'standard',
					title: 'Your privacy and how ads support ShroudEdit',
					text: 'Ads make ShroudEdit possible and fund creator rewards. Our partners may store unique identifiers to personalize ads and measure performance.',
					type: 'info',
					autoCloseMs: null,
					dismissible: false,
					buttons: [
