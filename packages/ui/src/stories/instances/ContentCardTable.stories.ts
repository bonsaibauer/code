import { DownloadIcon, EyeIcon, FolderOpenIcon } from '@shroudedit/assets'
...
const longNameItem: ContentCardTableItem = {
	...
	project: {
		...
		title: '[EMF] Entity Model Features - The Ultimate Entity Rendering Mod for Enshrouded',
		...
		template: /*html*/ `
			...
			>
				...
				<template #itemButtonsRight="{ item }">
					<IconButton type="quiet" label="View on ShroudEdit" v-tooltip="'View on ShroudEdit'" @click="console.log('View', item.id)">
	...
	render: () => ({
		...
		template: /*html*/ `
			<ContentCardTable
				...
				@delete="(id) => console.log('Delete', id)"
			>
				<template #view>View on ShroudEdit</template>
