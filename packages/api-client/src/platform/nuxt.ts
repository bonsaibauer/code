 ...
 * ```
 */
export class NuxtShroudEditClient extends XHRUploadClient {
	...
	upload<T = void>(path: string, options: UploadRequestOptions): UploadHandle<T> {
		// @ts-expect-error - import.meta is provided by Nuxt
		if (import.meta.server) {
			throw new ShroudEditApiError('upload() is not supported during SSR')
		}
	...
	protected async executeStreamRequest(
		...
		try {
			...
			if (!response.body) {
				throw new ShroudEditApiError('Streaming response has no readable body', {
	...
	protected normalizeError(error: unknown): ShroudEditApiError {
