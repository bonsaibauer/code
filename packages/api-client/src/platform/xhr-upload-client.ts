import { AbstractShroudEditClient } from '../core/abstract-client'
 ...
 *
 * Platform-specific clients should extend this instead of AbstractShroudEditClient
 * to inherit the XHR upload implementation.
 */
export abstract class XHRUploadClient extends AbstractShroudEditClient {
		...
		const handle: UploadHandle<T> = {
			promise: (async () => {
				...
				if (abortController.signal.aborted) {
					throw new ShroudEditApiError('Upload cancelled')
				}
	...
	protected executeXHRUpload<T>(
		...
		return new Promise<T>((resolve, reject) => {
			...
			xhr.addEventListener('error', () => reject(new ShroudEditApiError('Upload failed')))
			xhr.addEventListener('abort', () => reject(new ShroudEditApiError('Upload cancelled')))
			xhr.addEventListener('timeout', () => reject(new ShroudEditApiError('Upload timed out')))
	...
	protected createUploadError(xhr: XMLHttpRequest): ShroudEditApiError {
